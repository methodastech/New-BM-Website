...
(function(){
  ...
  /* --- depth grid floor (the "infrastructure" plane) --- */
  const grid = new THREE.GridHelper(60, 60, 0x6a0c16, 0x340810);
  grid.material.transparent = true; grid.material.opacity = 0.32; grid.position.y = -2.6;
  scene.add(grid);
  ...
  scene.add(new THREE.Line(
    new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(RAIL_A,0,0), new THREE.Vector3(RAIL_B,0,0)]),
    new THREE.LineBasicMaterial({ color:REDD, transparent:true, opacity:0.62 })));
  // faint parallel under-track for a subtle rail/depth read
  scene.add(new THREE.Line(
    new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(RAIL_A,-0.14,0), new THREE.Vector3(RAIL_B,-0.14,0)]),
    new THREE.LineBasicMaterial({ color:0x5a0c14, transparent:true, opacity:0.38 })));
  const packets = [];
  {
    ...
    for(let k=0;k<N;k++){
      const p = new THREE.Mesh(new THREE.SphereGeometry(0.16, 16, 16), new THREE.MeshBasicMaterial({ color:0xffe2e4 }));
      scene.add(p);
  ...
  function size(){
    ...
    renderer.setPixelRatio(Math.min(devicePixelRatio||1, 2));   // re-read DPR (zoom / monitor move)
    ...
    // drop nodes below the heading on the tall full-bleed canvas; center them in the short mobile band
    lookY = (r.height > innerHeight*0.55) ? 2.45 : 0.35;
    camera.updateProjectionMatrix();
  }
  size(); window.addEventListener('resize', size, { passive:true });

  /* --- pointer parallax + hover-to-highlight --- */
  ...
  function place(t){
    ...
    packets.forEach(pk=>{
      ...
      const fade = Math.min(1, Math.sin(p*Math.PI)*1.4); // soft at the ends, gentle in the middle
      ...
      if(pk.trail) pk.trail.forEach(function(ts,j){
        ...
        const tf = Math.min(1, Math.sin(tp*Math.PI)*1.4) * (1 - (j+1)/5) * 0.3;
    ...
    scene.rotation.y = mx*0.16 + Math.sin(t*0.1)*0.03;
...
(function(){
  ...
  renderer.setPixelRatio(Math.min(devicePixelRatio||1, COARSE ? 1 : 1.5)); // small icons — half the framebuffer
  ...
  function size(){ renderer.setPixelRatio(Math.min(devicePixelRatio||1, COARSE ? 1 : 1.5)); renderer.setSize(window.innerWidth, window.innerHeight, false); }
  size(); window.addEventListener('resize', size, { passive:true });
  let active = true;
  function drawAll(){
  ...
  if(REDUCE){
    ...
    if(window.__lenis && window.__lenis.on) window.__lenis.on('scroll', redraw);
    else window.addEventListener('scroll', redraw, { passive:true });
    return;
  }
