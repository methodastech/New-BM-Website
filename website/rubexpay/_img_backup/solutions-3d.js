...
(function(){
  ...
  function makeShape(name){
    ...
    if(name === 'unified'){            // Unified Systems — a 2x2x2 cluster of cubes (many parts, one system)
      const sg = new THREE.BoxGeometry(0.62,0.62,0.62);
      [-0.43,0.43].forEach(x=>[-0.43,0.43].forEach(y=>[-0.43,0.43].forEach(z=>{ const m=new THREE.Mesh(sg, glassMat()); m.position.set(x,y,z); g.add(m, (function(){var e=edge(sg);e.position.set(x,y,z);return e;})()); })));
    ...
    if(name === 'data'){               // Data Control — wireframe shell + stacked analytics layers
      ...
      [-0.45,0,0.45].forEach((y,i)=>{ const pg=new THREE.BoxGeometry(1.2-i*0.12,0.1,1.2-i*0.12); const m=new THREE.Mesh(pg, new THREE.MeshStandardMaterial({ color:0x33060c, metalness:0.6, roughness:0.25, emissive:RED, emissiveIntensity:0.7 })); m.position.y=y; g.add(m); });
  ...
  function loop(now){
    ...
    shapes.forEach((b,i)=>{
      ...
      if(b.userData.ring) b.userData.ring.rotation.z = t*0.6;
